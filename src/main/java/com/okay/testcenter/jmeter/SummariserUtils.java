...
public class SummariserUtils extends AbstractTestElement implements Serializable, SampleListener, TestStateListener, NoThreadClone, Remoteable {
    ...
    protected StringBuffer timeOutData = new StringBuffer();
    ...
    public void sampleOccurred(SampleEvent e) {
       ...
       if(s.getTime() > 5000 && timeOutData.length() < 2) {
           getTimeOutMesssage(s);
       }

        if (!s.isSuccessful()) {
            String requestid = s.getRequestHeaders().split("requestid:")[1].trim().split("\n")[0].trim();
            log.error("ErrorRequestid==" + requestid);
            log.error("ErrorResponseCode==" + s.getResponseCode());
            log.error("ErrorResponseDataAsString==" + s.getResponseDataAsString());
            getErrMesssage(s);
        }
    ...
    public void testEnded(String host) {
        ...
        if (totals != null) {
            ...
            while (var8.hasNext()) {
                Entry<String, Totals> entry = (Entry) var8.next();
    ...
    public void getErrMesssage(SampleResult s){
        if(!errorData.toString().isEmpty()){
            return;
        }
         String requestid = s.getRequestHeaders().split("requestid:")[1].trim().split("\n")[0].trim();
         String response;
         String code = s.getResponseCode();
          if ("200".equals(code)){
               response = s.getResponseDataAsString();
          }else {
               response = s.getResponseDataAsString().split("at")[0];
          }
             errorData.append("requestid:" + requestid + "\n");
             errorData.append("responseCode:" + code + "\n");
             if(response.contains("<html>")){
                 response =  response.replace("<html>","");
             }
             errorData.append("respnseData:" + response + "\n");

        log.error("errorData==" + errorData.toString());
    }

    public void getTimeOutMesssage(SampleResult s){
        if(!timeOutData.toString().isEmpty()){
            return;
        }
        String requestid = s.getRequestHeaders().split("requestid:")[1].trim().split("\n")[0].trim();
        timeOutData.append("响应时间:" + s.getTime()+ "\n");
        timeOutData.append("requestid:" + requestid + "\n");
        log.error("timeOutData==" + timeOutData.toString());
    }

    public String  getErr(){
        if (errorData.toString().equals("") && timeOutData.toString().equals("")) {
            return "";
        }
