...
function addPlan() {
    ...
    $.ajax({
        type: "POST",
        url: "/perf/middle/per/insertPlanSet",
        dataType: "json",
...
function deleteInfo() {
    ...
    $.ajax({
        type: "Get",
        url: "/perf/middle/per/deletePlanSet?id=" + deleteId,
        dataType: "json",
        contentType: "application/json",
        timeout: 60000,
        success: function (data) {
...
function updatePlan() {
    ...
    $.ajax({
        type: "POST",
        url: "/perf/middle/per/updatePlanSet",
        dataType: "json",
        contentType: "application/json",
        data: JSON.stringify(requestData),
        timeout: 60000,
...
function runJmeterSet(obj) {
    ...
    var url = "/perf/middle/per/run/set?setId=" + setId;
    console.log("url==" + url);
    $("#respose").val("运行中...");
...
function updatePageData() {
    ...
    var url = "/perf/middle/per/set/getlist?currentPage=" + currentPage + "&pageSize=10";
...
function content(middleCase) {
    ...
    for (var i = 0; i < middleCase.length; i++) {
        ...
        contents += "<tr><td>" + middleCase[i].id + "</td>\n" +
            "                    <td>" + middleCase[i].plan_set + "</td>\n" +
            "                    <td>" + middleCase[i].set_name + "</td>\n" +
            "                    <td>\n" +
            "                        <a href=\"javascript:void(0)\"\n" +
            "                           data-set_id=\'" + middleCase[i].id + "\'\n" +
            "                           data-plan_set=\'" + middleCase[i].plan_set + "\'\n" +
            "                           data-set_name=\'" + middleCase[i].set_name + "\'\n" +
...
function search() {
    ...
    var url = "/perf/middle/per/set/findBySetName?setName=" + setName;
