...
function addPlan() {
    ...
    var moreUser = $.trim($('#addMoreUser').find("option:selected").text());
    ...
    $.ajax({
        type: "POST",
        url: "/perf/middle/per/insertPlan",
        dataType: "json",
...
function deleteInfo() {
    ...
    $.ajax({
        type: "Get",
        url: "/perf/middle/per/deletePlan?id=" + deleteId,
        dataType: "json",
        contentType: "application/json",
        timeout: 60000,
        success: function (data) {
...
function updatePlan() {
    ...
    var moreUser = $.trim($('#updateMoreUser').find("option:selected").text());
    var SAVEDATA = [];
    ...
    $.ajax({
        type: "POST",
        url: "/perf/middle/per/updatePlan",
        dataType: "json",
        contentType: "application/json",
        data: JSON.stringify(requestData),
        timeout: 60000,
...
function runPlanModal(obj) {
    ...
    var threads = $(obj).attr("data-threads");
    var times = $(obj).attr("data-times");
    var url = "/perf/middle/per/run/" + case_id + "/" + threads + "/" + times;
    console.log("url==" + url);
    $("#respose").val("运行中...");
    //显示模态框
    $("#runCase").modal({
...
function runJmeterPlanModal(obj) {
    ...
    var moreUser = $(obj).attr("data-moreUser");
    var slaveStr = "";
    var url = "/perf/middle/per/run/jmeter?caseId=" + case_id + "&threadNum=" + threads + "&runTime=" + times + "&moreUser=" + moreUser + "&slaveStr=" + slaveStr;
    console.log("url==" + url);
    $("#respose").val("运行中...");
    //显示模态框
    $("#runCase").modal({
...
function updatePageData() {
    ...
    var url = "/perf/middle/per/getlist?currentPage=" + currentPage + "&pageSize=10";
...
function content(middleCase) {
    ...
    for (var i = 0; i < middleCase.length; i++) {
        ...
        var id = middleCase[i].id;
        var caseId = middleCase[i].case_id;
        var caseName = middleCase[i].case_name;
        var threads = middleCase[i].threads;
        var times = middleCase[i].times;
        var moreUser = middleCase[i].moreUser;
...
function search() {
    ...
    var url = "/perf/middle/per/findByCaseId?caseId=" + caseId;
